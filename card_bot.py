...
async def aidentika_upload(image_b64: str) -> str:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        logger.error(f"Aidentika upload: {r.status_code} {r.text[:200]}")
        return ""
...
async def aidentika_generate_card(upload_id: str, product_name: str, features: str, style: str = "classic") -> str:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        logger.error(f"Aidentika generate_card: {r.status_code} {r.text[:200]}")
        return ""
...
def _card_main_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📦 Пакеты",   callback_data="show_packages"),
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("mp_"):
        ...
        await query.edit_message_text(
            ...
            "Для другого маркетплейса: `/audit ozon`",
            parse_mode='Markdown'
        )
        ...
        await query.edit_message_text(
            ...
            "Или с маркетплейсом: _кроссовки ozon_",
            parse_mode='Markdown'
        )
        ...
        await query.edit_message_text(
            "🛍️ *КарточникБот v2.0* — выбери действие:",
            parse_mode='Markdown',
...
async def semantics_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text(
            ...
            "`/semantics кроссовки ym` — для Яндекс Маркет",
            parse_mode='Markdown'
        )
        return
    args        = context.args
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ─── Основной шаг — получение фото или названия ───
    ...
    await update.message.reply_text(
        ...
        f"Например: _тёмный фон_, _премиум стиль_, _яркие цвета_\n\n"
...
def main():
        ...
        try:
            if YOUR_CHAT_ID:
                await application.bot.send_message(
                    chat_id=YOUR_CHAT_ID,
                    text=(
                        "🛍️ *Карточник v2.0 запущен!*\n\n"
                        "⚡️ Anthropic Claude Haiku\n"
                        "✅ Автосканирование каждые 30 мин\n"
                        "📨 Заказы идут через Лилу\n\n"
                        "🆕 Новые функции:\n"
                        "/audit — аудит карточки клиента\n"
                        "/semantics — ключевые слова\n"
                        "/ugc — отзывы и FAQ\n"
                        "/packages — пакеты со скидкой"
                    ),
                    parse_mode='Markdown'
                )
        except Exception as e:
    ...
    logger.info("🛍️ Карточник v2.0 запущен!")
